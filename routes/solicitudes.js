...
      "SELECT idContrato FROM solicitudes_cliente WHERE id = ? LIMIT 1", 
      [solicitud_id]
    );
...
    // 3️⃣ Obtener datos del contrato
    // <-- ¡CORREGIDO! Consulta SQL puesta en una sola línea para evitar errores 'ER_PARSE_ERROR'
    const [contratos] = await db.query(
      `SELECT cliente_id, plan_id, nombre_campana, fecha_inicio, fecha_fin, monto_acordado, detalles_anuncio, precio_base, descuento, dias_emision FROM contratos_publicitarios WHERE id = ?`, 
      [contratoId]
    );
